...
VAF <- as.data.frame(af_data[, "test_T"])  ## Change column name based on VCF corresponding to tumor sample!!!
colnames(VAF) <- c("VAF")

# Calculate read depth from Tumor AD
ad_extracted <- ad_df %>%
  mutate(Tumor_Sample = as.character(ad_data[, "test_T"])) %>%  ## Change column name based on VCF corresponding to tumor sample!!!
...
ad_extracted <- ad_extracted %>%
  mutate(Normal_Sample = as.character(ad_data[, "test_N"])) %>%  ## Change column name based on VCF corresponding to normal sample!!!
